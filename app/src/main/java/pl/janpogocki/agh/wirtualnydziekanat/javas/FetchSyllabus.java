...
public class FetchSyllabus {
    ...
    public FetchSyllabus() {
        ...
        if (fw.getResponseCode() / 100 == 2){
            ...
            String linkDepartment = URLdomainSyllabus + fwParsed.select("a.department-link.table-item-link:contains(" + nazwaWydzialu + ")").get(0).attr("href");
            ...
            if (fw.getResponseCode() / 100 == 2) {
                ...
                if (level.contains("pierwszego"))
                    Storage.syllabusURL = URLdomainSyllabus + fwParsed.select("a.table-item-link:contains(" + kierunek + ")").get(0).attr("href");
                else if (specjalizacja.length() > 0)
                    Storage.syllabusURL = URLdomainSyllabus + fwParsed.select("a.table-item-link:contains(" + kierunek + " - " + specjalizacja + ")").get(0).attr("href");
            }
        }
    }
}
